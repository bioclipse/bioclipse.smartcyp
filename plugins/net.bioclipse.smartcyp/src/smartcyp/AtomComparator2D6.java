...
public class AtomComparator2D6 implements Comparator<IAtom> {
    ...
    private int compareScore( IAtom currentAtom, IAtom comparisonAtom ) {
		...
		// Set Score values
		if(SMARTCYP_PROPERTY.Score2D6.get(currentAtom) != null)  currentAtomScore = SMARTCYP_PROPERTY.Score2D6.get(currentAtom).doubleValue();
		if(SMARTCYP_PROPERTY.Score2D6.get(comparisonAtom) != null)  comparisonAtomScore = SMARTCYP_PROPERTY.Score2D6.get(comparisonAtom).doubleValue();
		
		// Dual null Scores
		if (SMARTCYP_PROPERTY.Score2D6.get(currentAtom) == null && SMARTCYP_PROPERTY.Score2D6.get(comparisonAtom) == null){					
		...
		// Single null scores
		else if(SMARTCYP_PROPERTY.Score2D6.get(currentAtom) == null) return after;
		else if(SMARTCYP_PROPERTY.Score2D6.get(comparisonAtom) == null) return before;
