...
public class SMARTSnEnergiesTable {
	...
	public SMARTSnEnergiesTable() {
		...
		SMARTSnEnergiesTable.put("[$([S]=[C])]", 43.5); //new rule in 2.2
		...
		SMARTSnEnergiesTable.put("[$([CX4][#6^2]~[#8]),$([CX4][cr5]),$([CX4]([c])[c]),$([CX4]([c])[C]=[C]),$([CX4][#6^1]),$([CX4][C^2]=[C^2]-[#6^2]),$([CX4][NX3][N]=[O]);!$([CH0]);!$([CX4][C](=[O])[NX3]);!$([CX4][#6^2](=[#8])-[#8]);!$([CX4][C^2]([C^2])=[C^2]-[#6^2]);!$([CX4][#6^2](=[#8])[#6^2])]", 59.7);
		...
		SMARTSnEnergiesTable.put("[$([CX4][S][P]=[S]);!$([CH0]);!$([C][S]=[O])]", 66.1);
